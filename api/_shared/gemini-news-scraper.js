...
function fuzzyMatchPlayer(queryName, teamCode) {
    const normQuery = normalize(queryName);
    const teamPlayers = PLAYERS.filter(p => p.team === teamCode);
    // 1. Exact match
    for (const p of teamPlayers) {
        if (normalize(p.name) === normQuery)
            return p.id;
    }
    // 2. Substring match
    for (const p of teamPlayers) {
        const normPName = normalize(p.name);
        if (normPName.includes(normQuery) || normQuery.includes(normPName))
            return p.id;
    }
    ...
    for (const p of teamPlayers) {
        ...
        for (const word of words) {
            if (normPName.includes(word))
                return p.id;
        }
    }
    return null;
}
export async function scrapeCardsForMatch(homeCode, awayCode, geminiKey) {
    ...
    try {
        ...
        if (Array.isArray(raw.assists)) {
            for (const item of raw.assists) {
                ...
                if (id)
                    result.assists.push({ id, count: item.count || 1 });
            }
        }
        // Map saves
        if (Array.isArray(raw.saves)) {
            for (const item of raw.saves) {
                const id = fuzzyMatchPlayer(item.name, item.team);
                if (id)
                    result.saves.push({ id, count: item.count || 0 });
            }
        }
        return result;
    }
